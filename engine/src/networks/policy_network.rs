...
impl PolicyNetwork {
    pub fn create_base(&self, board: &ChessBoard) -> Accumulator<i16, { HL_SIZE / 2 }> {
        ...
        for (i, (&a, &b)) in result.values_mut().iter_mut().zip(
            inputs
                .values()
                .iter()
                .take(HL_SIZE / 2)
...
fn map_move_to_index(board: &ChessBoard, mv: Move, see: bool, _chess960: bool) -> usize {
    let hm = if board.king_square(board.side()).file() > 3 {
        7
    } else {
        0
    };
...
const PAWN: [u64; 64] = init!(|sq, 64| {
    let bit = 1 << sq;
    ((bit & !Bitboard::FILE_A.get_value()) << 7)
        | (bit << 8)
        | ((bit & !Bitboard::FILE_H.get_value()) << 9)
});
