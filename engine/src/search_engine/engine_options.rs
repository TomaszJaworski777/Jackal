use crate::{create_options, WDLScore};
...
create_options! {
    EngineOptions {
        Options {
            ...
            ["MoveOverhead"] move_overhead: i64   =>  10,  0,  2000;
        ...
        Tunables {
            ...
            //CPUCT
            start_cpuct:       f64  =>  0.780,  0.1,  5.0,  0.10,  0.002;
            ...
            //FPU
            fpu_reduction: f64  =>  0.20,  0.05,  0.95,  0.05,  0.002;
            ...
            //Material Scaling
            knight_value:       f64  =>  397.6990,   150.0,  750.0,   15.0,  0.002;
            bishop_value:       f64  =>  401.4182,   150.0,  750.0,   15.0,  0.002;
            rook_value:         f64  =>  758.7151,   400.0,  1000.0,  30.0,  0.002;
            queen_value:        f64  =>  1505.7605,  900.0,  2000.0,  30.0,  0.002;
            scale_start_pos:    f64  =>  0.6859,     0.5,    1.5,     0.05,  0.002;
            scale_zero_mat:     f64  =>  0.6051,     0.1,    1.0,     0.05,  0.002;
            material_power:     f64  =>  0.9812,     0.1,    5.0,     0.1,   0.002;
            wl_dampening_power: f64  =>  20.1599,    1.0,    50.0,    2.0,   0.002;
            ...
            tm_sac_scale:           f64  =>  0.003,   0.0,    0.1,   0.001,   0.002;
            ...
            //Transposition Table
            hash_size: f64  =>  0.1,  0.01,  0.5,  0.004,  0.002;
        }
        ConstVariables {
            draw_score:                      f64  =  0.30;
            ...
            selection_castle_bonus:          f64  =  0.015;
            exploration_castle_bonus:        f64  =  0.025;
            selection_pawn_push_bonus:       f64  =  0.002;
            exploration_pawn_push_bonus:     f64  =  0.003;
            selection_queen_trade_penalty:   f64  =  0.02;
            exploration_queen_trade_penalty: f64  =  0.02;
...
impl EngineOptions {
    pub fn get_draw_score_blend(&self, score: WDLScore) -> f64 {
        let q_mag = (score.single() - 0.5).abs() * 2.0;
        let blend = 1.0 / (1.0 + (-10.0 * (q_mag - 0.5)).exp());

        self.draw_score() + (0.5 - self.draw_score()) * blend
    }
}
