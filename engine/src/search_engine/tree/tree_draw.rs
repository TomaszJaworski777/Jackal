...
use crate::{
    search_engine::tree::{node::Node, GameState, NodeIndex, Tree},
    SearchEngine,
};

impl Tree {
    pub fn draw_tree<const FLIP_SCORE: bool>(
        &self,
        depth: Option<u8>,
        ...
        self.print_branch::<FLIP_SCORE>(
            ...
            false,
            true,
            node_depth.unwrap().is_multiple_of(2),
            0,
            20,
            policy,
            policy,
    ...
    fn print_node(
        ...
        let score = match state {
            ...
            _ => format!(
                "{}{:.2}",
                if wdl_score.single() < 0.5 { "-" } else { "+" },
                wdl_score.cp().abs() as f32 / 100.0
            ),
        };

        let score = heat_color(
            &score.align_to_right(6),
            wdl_score.single() as f32,
            0.0,
            1.0,
        );
        ...
        let policy = heat_color(
            &format!("{:.2}%", node.policy() * 100.0).align_to_right(6),
            node.policy() as f32,
            min_policy,
            max_policy,
        );
