...
use crate::search_engine::tree::node::{
    game_state::AtomicGameState,
    lock::{IndexLock, IndexLockGuard},
    wdl_score::SCORE_SCALE,
};
...
pub use wdl_score::{AtomicWDLScore, WDLScore};
...
impl Node {
    ...
    pub fn set_to(&self, node: &Node) {
        ...
        self.cumulative_score
            .store(node.cumulative_score.get_score());
        self.squared_score.store(
            node.squared_score.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.state.set(node.state());
        self.policy
            .store(node.policy.load(Ordering::Relaxed), Ordering::Relaxed);
    ...
    pub fn squared_score(&self) -> f64 {
        ((self.squared_score.load(Ordering::Relaxed) as f64) / f64::from(SCORE_SCALE))
            / f64::from(self.visits())
    }
    ...
    pub fn set_policy(&self, policy: f64) {
        self.policy
            .store((policy * f64::from(u16::MAX)) as u16, Ordering::Relaxed)
    }
    ...
    pub fn set_gini_impurity(&self, gini_impurity: f64) {
        self.gini_impurity.store(
            (gini_impurity * f64::from(u16::MAX)) as u16,
            Ordering::Relaxed,
        )
    }
    ...
    pub fn add_visit(&self, score: WDLScore) {
        ...
        self.squared_score.fetch_add(
            (score.powi(2) * f64::from(SCORE_SCALE)) as u64,
            Ordering::Relaxed,
        );
    }
