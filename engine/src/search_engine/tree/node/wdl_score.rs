...
impl From<WDLScore> for AtomicWDLScore {
    fn from(value: WDLScore) -> Self {
        let win_chance = (value.win_chance() * f64::from(SCORE_SCALE)) as u64;
        let draw_chance = (value.draw_chance() * f64::from(SCORE_SCALE)) as u64;

        Self(AtomicU64::new(win_chance), AtomicU64::new(draw_chance))
    }
}
...
impl AtomicWDLScore {
    ...
    pub fn add(&self, rhs: WDLScore) {
        let win_chance = (rhs.win_chance() * f64::from(SCORE_SCALE)) as u64;
        let draw_chance = (rhs.draw_chance() * f64::from(SCORE_SCALE)) as u64;
...
impl WDLScore {
    ...
    pub fn apply_50mr_and_draw_scaling(
        &mut self,
        half_move: u8,
        depth: f64,
        options: &EngineOptions,
    ) {
        let s = (0.01 * half_move as f64)
            .powf(options.power_50mr())
            .min(options.cap_50mr())
            + (depth.powf(options.depth_scaling_power()) * options.depth_scaling())
                .min(options.depth_scaling_cap());
    ...
    pub fn apply_draw_pessimism(&mut self, board: &ChessBoard, options: &EngineOptions) {
        let penalty = options.draw_pessimism();
        if penalty == 0.0 || board.phase() <= 4 {
            return;
        }

        let phase_factor = 1.0 - ((board.phase() as f64 - 4.0) / 20.0).clamp(0.0, 1.0);
        let effective_penalty = penalty * phase_factor;

        let draw_delta = self.draw_chance() * effective_penalty;
        self.1 -= draw_delta;
    }
    ...
    pub fn apply_contempt(&mut self, contempt: i64) {
        ...
        let shift = (uncertainty.powi(2) * contempt as f64 * std::f64::consts::LN_10
            / (400.0 * 16.0))
            .clamp(-0.8, 0.8);
...
impl Mul<u32> for WDLScore {
    ...
    fn mul(self, rhs: u32) -> Self::Output {
        Self(
            self.win_chance() * f64::from(rhs),
            self.draw_chance() * f64::from(rhs),
        )
    }
}
