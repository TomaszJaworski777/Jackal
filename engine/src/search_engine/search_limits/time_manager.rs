...
impl TimeManager {
    pub fn calculate_time_limit(
        ...
        let time_left =
            (time_remaining as f64 + increment as f64 * (mtg - 1.0) - 10.0 * (2.0 + mtg)).max(1.0);
        let log_time = (time_left / 1000.0).log10();

        let phase = (1.0 - (phase / 24.0))
            .powf(options.phase_power())
            .clamp(0.0, 1.0)
            * options.phase_scale();

        let soft_constant = (options.soft_constant() + options.soft_constant_multi() * log_time)
            .min(options.soft_constant_cap());
        let soft_scale = (options.soft_scale()
            + (game_ply as f64 + options.soft_scale_offset() + phase).sqrt() * soft_constant)
            .min(options.soft_scale_cap() * time_remaining as f64 / time_left);

        let hard_constant = (options.hard_constant() + options.hard_constant_multi() * log_time)
            .max(options.hard_constant_cap());
        let hard_scale = (hard_constant + game_ply as f64 / options.hard_ply_div())
            .min(options.hard_scale_cap());

        let bonus = 1.0
            + options.tm_bonus_scale()
                * (1.0
                    + options.tm_bonus_move_factor()
                        * (-(game_ply as f64 / options.tm_bonus_ply_div())
                            .powf(options.tm_bonus_power()))
                        .exp())
                .log10();
    ...
    pub fn soft_limit_reached(
        ...
        soft_limit_multiplier *=
            self.visits_distribution(best_idx, second_best_idx, iterations, tree, options);

        if let Some(best_idx) = best_idx {
            soft_limit_multiplier *= self.falling_eval(best_idx, iterations, tree, options);
            soft_limit_multiplier *= self.when_behind(best_idx, iterations, tree, options);
        }

        soft_limit_multiplier *=
            self.best_move_instability(iterations, tree, options, best_move_changes);

        soft_limit_multiplier *= self.sacrifice_extension(best_idx, second_best_idx, tree, options);

        elapsed_ms
            >= ((self.soft_limit.unwrap() as f64 * soft_limit_multiplier) as u128)
                .saturating_sub(move_overhead)
                .max(1)
    }

    fn visits_distribution(
        &mut self,
        best_idx: Option<NodeIndex>,
        second_best_idx: Option<NodeIndex>,
        iterations: u64,
        tree: &Tree,
        options: &EngineOptions,
    ) -> f64 {
        ...
        let visit_gap_ratio = ((best_move_visits.saturating_sub(second_move_visits)) as f64).abs()
            / tree.root_node().visits() as f64
            - options.gap_threshold();

        let visit_gap = if visit_gap_ratio > 0.0 {
            2.0 * options.gap_reward_scale()
                / (1.0 + (options.gap_reward_multi() * visit_gap_ratio).exp())
                - options.gap_reward_scale()
        } else {
            2.0 * options.gap_penalty_scale()
                / (1.0 + (options.gap_penalty_multi() * visit_gap_ratio).exp())
                - options.gap_penalty_scale()
        };

        let visit_difference_ratio = best_move_visits as f64 / tree.root_node().visits() as f64
            - options.visit_distr_threshold();

        let time_multiplier = if visit_difference_ratio > 0.0 {
            let reward_scale = options.visit_reward_scale() + visit_gap;
            2.0 * reward_scale
                / (1.0 + (options.visit_reward_multi() * visit_difference_ratio).exp())
                - reward_scale
        } else {
            let penalty_scale = options.visit_penalty_scale() + visit_gap;
            2.0 * penalty_scale
                / (1.0 + (options.visit_penalty_multi() * visit_difference_ratio).exp())
                - penalty_scale
        };
    ...
    fn falling_eval(
        ...
        let current_score = tree[best_idx].score().cp() as f64 / 100.0;
        ...
        let multiplier = curve(
            score_trend + 0.5,
            options.falling_eval_power(),
            options.falling_eval_multi(),
        )
        .clamp(
            -options.falling_reward_clamp(),
            options.falling_penalty_clamp(),
        );

        1.0 + multiplier
    }

    fn best_move_instability(
        &mut self,
        iterations: u64,
        tree: &Tree,
        options: &EngineOptions,
        ...
        let move_changes = if let Some(mut previous_changes) = self.previous_best_move_changes {
            previous_changes +=
                options.instability_ema_alpha() * (best_move_changes as f64 - previous_changes);
        ...
        let multiplier = (options.instability_multi() * move_changes)
            .powi(2)
            .min(1.0)
            * options.instability_scale();
    ...
    fn when_behind(
        ...
        let multiplier =
            (options.behind_multi() * -current_score).powi(2).min(1.0) * options.behind_scale();

        1.0 + multiplier
    }

    fn sacrifice_extension(
        &self,
        best_idx: Option<NodeIndex>,
        second_best_idx: Option<NodeIndex>,
        tree: &Tree,
        ...
        if let Some(best_idx) = best_idx {
            max_sac = max_sac.max(tree[best_idx].sac_strength());
        }

        if let Some(second_best_idx) = second_best_idx {
            max_sac = max_sac.max(tree[second_best_idx].sac_strength());
        }
        ...
        1.0 + options.tm_sac_scale() * max_sac as f64
    }
}
