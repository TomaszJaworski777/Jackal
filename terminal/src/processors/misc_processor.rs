...
use utils::{
    ...
    AlignString, Colors, CustomColor, PieceColors, Theme, DRAW_COLOR, LOSE_COLOR, WIN_COLOR,
};
        ...
        match command {
            ...
            "tree" => {
                ...
                let node_idx = if args.len() >= 3 {
                    ...
                    if let (Some(h), Some(i)) = (half, idx) {
                        Some(NodeIndex::new(h, i))
                    } else {
                        None
                    }
                } else {
                ...
                search_engine
                    .tree()
                    .draw_tree::<true>(depth, node_idx, search_engine);
            }
            "rawtree" => {
                let depth = if !args.is_empty() {
                    args[0].parse::<u8>().ok()
                ...
                search_engine
                    .tree()
                    .draw_tree::<false>(depth, node_idx, search_engine);
            }
            "perft" => {
                let depth = if !args.is_empty() {
                    args[0].parse::<u8>().ok()
...
fn draw_policy(search_engine: &SearchEngine) {
    ...
    for (name, network) in [
        ...
        board.map_legal_moves(|mv| {
            ...
            let p = network.forward(
                board,
                &policy_base,
                mv,
                see,
                search_engine.options().chess960(),
            );
            max = max.max(p);
            moves.push((mv, p));
        });
        ...
        for (idx, &(mv, p)) in moves.iter().enumerate() {
            println!(
                " {} {}",
                format!("{}:", mv.to_string(search_engine.options().chess960()))
                    .align_to_left(6)
                    .primary((idx as f32 + 10.0) / (moves.len() as f32 + 18.0)),
...
fn print_network_eval(board: &ChessBoard, network: &ValueNetwork, search_engine: &SearchEngine) {
    ...
    println!(
        ...
        format!(
            ...
            format!(
                ...
                heat_color(
                    if raw_score_cp > 0 { "+" } else { "-" },
                    raw_score.single() as f32,
                    0.0,
                    1.0
                ),
                heat_color(
                    format!("{:.2}", raw_score_cp.abs() as f32 / 100.0).as_str(),
                    raw_score.single() as f32,
                    0.0,
                    1.0
                ),
    ...
    println!(
        ...
        format!(
            ...
            format!(
                ...
                heat_color(
                    if half_moves_cp > 0 { "+" } else { "-" },
                    half_moves.single() as f32,
                    0.0,
                    1.0
                ),
                heat_color(
                    format!("{:.2}", half_moves_cp.abs() as f32 / 100.0).as_str(),
                    half_moves.single() as f32,
                    0.0,
                    1.0
                ),
    ...
    let mut draw_pessimism = raw_score;
    draw_pessimism.apply_draw_pessimism(board, search_engine.options());
    let draw_pessimism_cp = draw_pessimism.cp();
    ...
    println!(
        ...
        format!(
            ...
            format!(
                "[{}, {}, {}] ({}{})",
                format!("{:.2}%", draw_pessimism.win_chance() * 100.0).custom_color(WIN_COLOR),
                format!("{:.2}%", draw_pessimism.draw_chance() * 100.0).custom_color(DRAW_COLOR),
                format!("{:.2}%", draw_pessimism.lose_chance() * 100.0).custom_color(LOSE_COLOR),
                heat_color(
                    if draw_pessimism_cp > 0 { "+" } else { "-" },
                    draw_pessimism.single() as f32,
                    0.0,
                    1.0
                ),
                heat_color(
                    format!("{:.2}", draw_pessimism_cp.abs() as f32 / 100.0).as_str(),
                    draw_pessimism.single() as f32,
                    0.0,
                    1.0
                ),
    ...
    println!(
        ...
        format!(
            ...
            format!(
                ...
                heat_color(
                    if contempt_cp > 0 { "+" } else { "-" },
                    contempt.single() as f32,
                    0.0,
                    1.0
                ),
                heat_color(
                    format!("{:.2}", contempt_cp.abs() as f32 / 100.0).as_str(),
                    contempt.single() as f32,
                    0.0,
                    1.0
                ),
    ...
    total_score.apply_draw_pessimism(board, search_engine.options());
    ...
    println!(
        ...
        format!(
            ...
            format!(
                ...
                heat_color(
                    if total_score_cp > 0 { "+" } else { "-" },
                    total_score.single() as f32,
                    0.0,
                    1.0
                ),
                heat_color(
                    format!("{:.2}", total_score_cp.abs() as f32 / 100.0).as_str(),
                    total_score.single() as f32,
                    0.0,
                    1.0
                ),
...
fn analyse(search_engine: &mut SearchEngine, iters: Option<u64>) {
    ...
    println!(
        "\n{} {}",
        " FEN:   ".primary(0.0 / 32.0),
        FEN::from(&board).to_string().secondary(0.0 / 32.0)
    );
    println!(
        "{} {}\n",
        " Nodes: ".primary(1.0 / 32.0),
        iters.to_string().secondary(1.0 / 32.0)
    );
    ...
    print!(
        "{} {}\r",
        " Progress:".primary(4.0 / 32.0),
        create_loading_bar(50, progress, (225, 225, 225), (225, 225, 225)).secondary(4.0 / 32.0)
    );
    ...
    print!(
        "{} {}\r",
        " Progress:".primary(5.0 / 32.0),
        create_loading_bar(
            50,
            progress / piece_count as f32,
            (225, 225, 225),
            (225, 225, 225)
        )
        .secondary(5.0 / 32.0)
    ...
    let wdl_score = search_engine
        .tree()
        .get_best_pv(0, search_engine.options())
        .score();
    ...
    board.occupancy().map(|square| {
        ...
        if board_cpy.is_square_attacked(
        ...
        evals[usize::from(square)] = search_engine
            .tree()
            .get_best_pv(0, search_engine.options())
            .score()
            .cp();
    ...
    for rank in 0..9 {
        ...
        let square_data = |row_idx: u8, rank: u8, file: u8| -> String {
            ...
            match row_idx {
                0 => {
                    ...
                    format!("{}{}{}", rank_str, " ".repeat(5), file_char)
                        .align_to_center(7)
                        .gray()
                }
                1 => {
        ...
        for row_idx in 0..3 {
            for temp_file in 0..8 {
                let square_rank = if board.side() == Side::WHITE {
                    7 - rank
                } else {
                    rank
                };
                let square_file = if board.side() == Side::WHITE {
                    temp_file
                } else {
                    7 - temp_file
                };
                print!(
                    ...
                    square_data(row_idx, square_rank, square_file)
                );
                if temp_file == 7 {
                    println!(
                        "{}   {}",
