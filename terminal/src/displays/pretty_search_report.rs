...
use utils::{
    bytes_to_string, clear_terminal_screen, create_loading_bar, heat_color, number_to_string,
    time_to_string, AlignString, Theme, DRAW_COLOR, LOSE_COLOR, WIN_COLOR,
};
...
impl SearchReport for PrettySearchReport {
    ...
    fn search_ended(
        ...
        let draw_score = search_engine.options().draw_score();
        ...
        if let Ok((x, y)) = term_cursor::get_pos() {
...
fn print_search_report<const FINAL: bool>(
    _: &SearchLimits,
    search_stats: &SearchStats,
    search_engine: &SearchEngine,
) {
    let grad = |a: u8| -> f32 { a as f32 / 33.0 };

    let (t_width, t_height) = term_size::dimensions().unwrap_or((80, 25));
    ...
    if t_height >= 25 {
        ...
        println!(
            ...
            format!(
                " Tree Size:  {} | {}",
                format!("{}n", number_to_string(tree_size as u128)).secondary(grad(13)),
                format!("{}B", tree_bytes).secondary(grad(13))
            )
            .primary(grad(13))
        );
        print!("{}\r", " ".repeat(t_width));
        println!(
            ...
            format!(
                " Tree Usage: {}",
                create_loading_bar(50, usage, WIN_COLOR, LOSE_COLOR).secondary(grad(14))
            )
            .primary(grad(14))
        );
    ...
    if t_height >= 21 {
        ...
        println!(
            ...
            format!(
                " Nodes:      {}",
                number_to_string(nodes as u128).secondary(grad(18))
            )
            .primary(grad(18))
        );
        print!("{}\r", " ".repeat(t_width));
        println!(
            "{}",
            format!(" Time:       {}", time_to_string(time).secondary(grad(19))).primary(grad(19))
        );
        print!("{}\r", " ".repeat(t_width));
        println!(
            "{}",
            format!(" Nps:        {}", number_to_string(nps).secondary(grad(20))).primary(grad(20))
        );
    ...
    let score = match pv.first_node().state() {
        ...
        _ => format!(
            "{}{:.2}",
            if pv_score.single() < 0.5 { "-" } else { "+" },
            pv_score.cp().abs() as f32 / 100.0
        ),
    };
    ...
    println!(
        ...
        format!(
            " Score:      {}",
            heat_color(score.as_str(), pv_score.single() as f32, 0.0, 1.0)
        )
        .primary(grad(22))
    );
    print!("{}\r", " ".repeat(t_width));
    println!(
        ...
        format!(
            " Win:        {}",
            create_loading_bar(50, wdl.win_chance() as f32, WIN_COLOR, WIN_COLOR)
                .secondary(grad(23))
        )
        .primary(grad(23))
    );
    ...
    println!(
        ...
        format!(
            " Draw:       {}",
            create_loading_bar(50, wdl.draw_chance() as f32, DRAW_COLOR, DRAW_COLOR)
                .secondary(grad(24))
        )
        .primary(grad(24))
    );
    ...
    println!(
        ...
        format!(
            " Lose:       {}",
            create_loading_bar(50, wdl.lose_chance() as f32, LOSE_COLOR, LOSE_COLOR)
                .secondary(grad(25))
        )
        .primary(grad(25))
    );
    ...
    unsafe {
        ...
        if SEARCH_HISTORY.is_empty()
            || SEARCH_HISTORY.last().unwrap().1.first_move() != pv.first_move()
        {
    ...
    println!(
        "{}",
        format!(" Best Line:  {}", pv_string.secondary(grad(26))).primary(grad(26))
    );
    print!("{}\r", " ".repeat(t_width));
    println!();
    ...
    print!("{}\r", " ".repeat(t_width));
    println!("{}", " Search History:".to_string().primary(grad(28)));

    #[allow(static_mut_refs)]
    unsafe {
        let start_idx = (SEARCH_HISTORY.len() as i32
            - t_height.saturating_sub(height_used + 4) as i32)
            .max(0) as usize;
        for (time, pv) in &SEARCH_HISTORY[start_idx..] {
            ...
            println!(
                ...
                format!(
                    "{} -> {}",
                    time_to_string(*time).align_to_right(9),
                    pv_string
                )
                .secondary(grad(29))
            )
