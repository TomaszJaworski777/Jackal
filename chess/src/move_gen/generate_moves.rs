...
impl ChessBoard {
    ...
    pub fn has_legal_moves(&self) -> bool {
        if self.side() == Side::WHITE {
            self.has_legal_moves_internal::<WHITE>()
        } else {
            self.has_legal_moves_internal::<BLACK>()
        }
    }

    fn has_legal_moves_internal<const COLOR: u8>(&self) -> bool {
        let attack_map = self.generate_attack_map(Side::from(COLOR).flipped());
        let king_square = self.king_square(Side::from(COLOR));
        let (bishop_pins, rook_pins) = self.generate_pin_masks(Side::from(COLOR));
        let checkers = if attack_map.get_bit(king_square) {
        ...
        if MoveGen::count_king_moves::<COLOR>(self, attack_map, king_square) > 0 {
            return true;
        }

        let (push_map, capture_map) = if checkers.is_empty() {
            let push_map = !self.occupancy();
            let capture_map = self.occupancy_for_side(Side::from(COLOR).flipped());

            if MoveGen::count_castle_moves::<COLOR>(self, attack_map, king_square, rook_pins) > 0 {
                return true;
            }

            (push_map, capture_map)
        } else if (checkers & (checkers - 1)).is_empty() {
            let checker = checkers.ls1b_square();
            let push_map = Rays::get_ray(king_square, checker).exclude(checker);
        ...
        if MoveGen::count_pawn_moves::<COLOR>(self, push_map, capture_map, bishop_pins, rook_pins)
            > 0
        {
            return true;
        }

        if MoveGen::count_piece_moves::<COLOR, { KNIGHT }>(
            self,
            push_map,
            capture_map,
            bishop_pins,
        ...
        if MoveGen::count_piece_moves::<COLOR, { BISHOP }>(
            self,
            push_map,
            capture_map,
            bishop_pins,
        ...
        MoveGen::count_piece_moves::<COLOR, { ROOK }>(
            self,
            push_map,
            capture_map,
            bishop_pins,
