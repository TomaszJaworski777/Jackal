...
use crate::{clear_terminal_screen, search::Score, utils::{heat_color, lerp_color}, EngineOptions, GameState, SearchLimits, SearchStats};
...
impl SearchDisplay for PrettyPrint {
    fn new(position: &ChessPosition, engine_options: &EngineOptions) -> Self {
        ...
        println!(" {}  {}MB", "Tree Size:".bright_black(), engine_options.hash());
        ...
        let term_height = term_size::dimensions().expect("Cannot obtain terminal size").1;
    ...
    fn print_search_raport<const FINAL: bool>(
        ...
        print!("                                                \r");
        println!(" {} {}\n", "Tree Usage:".bright_black(), usage_bar(50, usage));

        print!("                                    \r");
        println!(" {} {}", "Avg. Depth:".bright_black(), search_stats.avg_depth());
        print!("                                    \r");
        println!(" {}  {}\n", "Max Depth:".bright_black(), search_stats.max_depth());

        print!("                                    \r");
        println!(" {}      {}", "Nodes:".bright_black(), StringUtils::large_number_to_string(search_stats.iters() as u128));
        print!("                                    \r");
        println!(" {}       {}", "Time:".bright_black(), StringUtils::time_to_string(search_stats.time_passed() as u128));
        print!("                                    \r");
        let nps = search_stats.iters() as u64 * 1000 / search_stats.time_passed().max(1);
        println!(" {}        {}\n", "Nps:".bright_black(), StringUtils::large_number_to_string(nps as u128));
        ...
        println!(" {}      {}", "Score:".bright_black(), heat_color(score_cp_string.as_str(), f32::from(score), 0.0, 1.0));
        print!("                                    \r");
        println!(" {}        {}%W {}%D {}%L", "WDL:".bright_black(), 75, 7, 13);
        print!("                                                                                                             \r");
        ...
        for idx in start_idx..self.history.len() {
            ...
            println!("  {} -> {}", pad_str(&StringUtils::time_to_string(*timestamp), 7, console::Alignment::Right, None).bright_black(), pv_line)
        }
        println!("                                                  ");

        if pv.len() > 0 && pv[0] != self.last_best_move {
            self.last_best_move = pv[0];
            self.history.push((search_stats.time_passed() as u128, pv_string));
        }
    }
...
fn pv_to_string<const FINAL: bool>(pv: &[Move]) -> String {
    ...
    for idx in 0..pv_length {
        pv_string.push_str(&lerp_color(&format!("{} ", pv[idx]), start_pv_color, end_pv_color, idx as f32 / 14.0));
    }
