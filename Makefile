...
default:
	$(NATIVE_HEADER) --bin jackal -- --emit link=$(DEV_NAME)$(EXT)

release: create_version_dir
	$(X86_64_v2_HEADER) --bin jackal -- --emit link=$(X86_64_V2)$(EXT)
	$(X86_64_v3_HEADER) --bin jackal -- --emit link=$(X86_64_V3)$(EXT)
	$(X86_64_v4_HEADER) --bin jackal -- --emit link=$(X86_64_V4)$(EXT)

gen:
	$(NATIVE_HEADER) -p datagen -- --emit link=datagen$(EXT)

trainer:
	$(NATIVE_HEADER) -p train -- --emit link=trainer$(EXT)
