...
fn main() {
    ...
    let openings = std::io::BufReader::new(
        File::open("./resources/books/DFRC.epd").expect("Book does not exist!"),
    )
    .lines()
    ...
    std::thread::scope(|s| {
        for _ in 0..threads {
            s.spawn(|| {
                ...
                _ = engine.set_option("Contempt", "100");
                _ = engine.set_option("DrawScore", "40");
                _ = engine.set_option("PolicySac", "2");
                engine.options_mut().set_kld_min(0.00000222);
                ...
                loop {
                    ...
                    datagen_stats.add_game(
                        game.moves.len() as u64,
                        (game.result * 2.0) as i32 - 1,
                        avg_iters,
                    );
        ...
        loop {
            ...
            let seconds_left =
                (target_positions as u128 - positions as u128) / speed.max(1) as u128;

            println!(
                "Progress:   {}",
                create_loading_bar(40, positions as f32 / target_positions as f32, WHITE, WHITE)
            );
            ...
            println!(
                "Games:      {}",
                utils::number_to_string(datagen_stats.games() as u128)
            );
            println!(
                "WDL:        [W: {}, D: {}, L: {}]\n",
